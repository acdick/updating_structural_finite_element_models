 ...
 * file for a Modal Assurance Criterion update.  The mode connections make up
 ...
 ******************************************************************************/
public class ModalAssuranceCriterionUpdateWriter extends FileWriter
    ...
    /***************************************************************************
     * Constructs a {@code ModalAssuranceCriterionUpdateWriter} from the given
     * file.
     * 
     * @param   file                the file to be exported
     * @throws  java.io.IOException if the file could not be written
     **************************************************************************/
    public ModalAssuranceCriterionUpdateWriter(File file)
            throws IOException {
        super(file);
    } // eom
    ...
    protected void writeCorrelationFunction(int modeIndex) {
        ...
        modeCorrelations    = firstModes.getReducedDotProduct(firstModes,
                firstModeKeys, firstNodeKeys, firstModeKeys, firstNodeKeys);
    ...
    protected void writeHeader() {
        ...
        this.writeComment("vom  .................... : 13.09.2008");
    ...
    protected void writeLastProductEquation(int modeIndex) {
        ...
        for (int i = 0; i < nNodes - 1; i++) {
            printWriter_.format("%-8s", "+");
            printWriter_.format("%-8s", "");
            printWriter_.format("M%02dDx%03d**2+", modeIndex + 1, i + 1);
            printWriter_.format("M%02dDy%03d**2+", modeIndex + 1, i + 1);
            printWriter_.format("M%02dDz%03d**2+", modeIndex + 1, i + 1);
            printWriter_.format("%n");
        }
        
        printWriter_.format("%-8s", "+");
        printWriter_.format("%-8s", "");
        printWriter_.format("M%02dDx%03d**2+", modeIndex + 1, nNodes);
        printWriter_.format("M%02dDy%03d**2+", modeIndex + 1, nNodes);
        printWriter_.format("M%02dDz%03d**2", modeIndex + 1, nNodes);
        printWriter_.format("%n");
    } // eom
    ...
    protected void writeLastProductNames(int modeIndex) {
        ...
        printWriter_.format("%8s", equationIdentifier_ - 1);
        printWriter_.format("%n");
        ...
        for (int i = 0; i < 3 * nNodes; i++) {
            ...
            printWriter_.format("%8d", entryIdentifier_ - (3 * nNodes) - 4 + i);
            iColumn++;
        }
    ...
    /***************************************************************************
     * Writes the equation of the {@code First Mode} and {@code Last Mode} for
     * the mixed product of the objective function at the given mode index.
     * 
     * @param   modeIndex   the index of the mode
     **************************************************************************/
    protected void writeMixedProductEquation(int modeIndex) {
        ...
        for (int i = 0; i < nNodes - 1; i++) {
            ...
            printWriter_.format("%-8s", "+");
            printWriter_.format("%-8s", "");
            printWriter_.format("% E*M%02dDx%03d+", x, modeIndex + 1, i + 1);
            printWriter_.format("%n");
            printWriter_.format("%-8s", "+");
            printWriter_.format("%-8s", "");
            printWriter_.format("% E*M%02dDy%03d+", y, modeIndex + 1, i + 1);
            printWriter_.format("%n");
            printWriter_.format("%-8s", "+");
            printWriter_.format("%-8s", "");
            printWriter_.format("% E*M%02dDz%03d+", z, modeIndex + 1, i + 1);
            printWriter_.format("%n");
        }
        
        x = firstModes.getXTranslationAt(modeIndex, nNodes - 1);
        y = firstModes.getYTranslationAt(modeIndex, nNodes - 1);
        z = firstModes.getZTranslationAt(modeIndex, nNodes - 1);
        
        printWriter_.format("%-8s", "+");
        printWriter_.format("%-8s", "");
        printWriter_.format("% E*M%02dDx%03d+", x, modeIndex + 1, nNodes);
        printWriter_.format("%n");
        printWriter_.format("%-8s", "+");
        printWriter_.format("%-8s", "");
        printWriter_.format("% E*M%02dDy%03d+", y, modeIndex + 1, nNodes);
        printWriter_.format("%n");
        printWriter_.format("%-8s", "+");
        printWriter_.format("%-8s", "");
        printWriter_.format("% E*M%02dDz%03d", z, modeIndex + 1, nNodes);
        printWriter_.format("%n");
    } // eom
    ...
    protected void writeMixedProductNames(int modeIndex) {
        ...
        for (int i = 0; i < 3 * nNodes; i++) {
            ...
            printWriter_.format("%8d", entryIdentifier_ - (3 * nNodes) - 3 + i);
            iColumn++;
        }
    ...
    protected void writeModeConnection(int modeConnectionIndex) {
        ...
        this.writeComment("Phi(Versuch) * Phi(Simulation)");
        this.writeComment("Mode: " + modeName);
        this.writeBreak();
        ...
        this.writeDoubleBreak();
        this.writeComment("Gleichung des Skalarprodukts");
        this.writeComment("Phi(Simulation) * Phi(Simulation)");
        this.writeComment("Mode: " + modeName);
        this.writeBreak();
        ...
        this.writeBreak();
        this.writeDoubleBreak();
        this.writeComment("MAC-Wert Versuch-Rechnung");
        this.writeComment("Mode: " + modeName);
        this.writeBreak();
    ...
    protected void writeObjectiveEquation() {
        ...
        for (int i = 0; i < nModes; i++) {
            ...
            printWriter_.format("xMac%02d,", i + 1);
            printWriter_.format("%n");
        }
        ...
        for (int i = 0; i < nModes - 1; i++) {
            ...
            printWriter_.format("Mac%02d,", i + 1);
            printWriter_.format("%n");
        }
        ...
        printWriter_.format("Mac%02d)=", nModes);
        printWriter_.format("%n");
        ...
        for (int i = 0; i < nModes - 1; i++) {
            ...
            printWriter_.format("xMac%02d*(Mac%02d-1.)**2+", i + 1, i + 1);
            printWriter_.format("%n");
        }
        ...
        printWriter_.format("xMac%02d*(Mac%02d-1.)**2", nModes, nModes);
        printWriter_.format("%n");
    ...
    protected void writeObjectiveFunction() {
        ...
        for (int i = 0; i < nModes; i++) {
            ...
            objectiveNames[(2 * i) + 1] = String.format("  xMac%02d", i + 1);
    ...
    protected void writeObjectiveWeights() {
        ...
        printWriter_.format("  xMac%02d", nModes);
    ...
    protected void writeSummary() {
        ...
        for (int i = 0; i < nModes; i++) {
            correlationLine = String.format(
                    "MAC-Wert            Versuchsmode (%2d)= DRESP-Id: %8d",
