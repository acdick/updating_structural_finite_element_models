...
/*******************************************************************************
 * This {@code Action} opens a dialog for the user to select the mesh input file
 * and then imports it.  It then checks if the nodes and node key groups that
 * were just imported have the same names.  If the check is {@code false}, it
 * displays a warning to the user.
 * 
 * @author  Adam C. Dick, BSE
 * @version September 13, 2008
 ******************************************************************************/
public class MeshOpenAction extends AbstractAction {
     ...
     *                      for
     **************************************************************************/
    public MeshOpenAction(JMeshPanel meshPanel, DefaultMeshUI meshUI) {
        super("Open...");
        
        meshPanel_  = meshPanel;
        meshUI_     = meshUI;
        
        this.putValue(SHORT_DESCRIPTION, "Open mesh file");
    } // eom
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        ...
        try {
            fileName = meshUI_.showMeshOpenDialog();
            if (fileName != null) {
                meshUI_.setMeshInput(fileName);
                meshPanel_.importGeometry(fileName);
